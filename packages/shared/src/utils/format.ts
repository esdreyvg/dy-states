...
export const formatCurrency = (
  amount: number,
  currency: 'DOP' | 'USD' | 'EUR' = 'DOP',
  locale: 'es-DO' | 'en-US' = 'es-DO'
): string => {
  try {
...
export const formatFeatures = (features: string[]): string => {
  if (features.length === 0) return 'Sin características especificadas';
  if (features.length === 1) return features[0];
  if (features.length === 2) return features.join(' y ');
